...
const OrdersView: React.FC<OrdersViewProps> = ({ orders, users, currentUser, onReprint, onRefund, storeSettings }) => {
                    ...
                    <tbody>
                        {paginatedOrders.map(order => (
                            <tr key={order.id} className="bg-white dark:bg-slate-800 border-b dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700">
                                ...
                                <td className="px-6 py-4 text-right font-semibold">{formatCurrency(order.total, storeSettings)}</td>
